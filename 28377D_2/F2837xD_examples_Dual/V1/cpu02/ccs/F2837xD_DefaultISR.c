...
interrupt void SCIA_RX_ISR(void)
{
	...
	// 若接收FIFO内不为空，则顺序取出存于循环队列rdataA[64]
	while(SciaRegs.SCIFFRX.bit.RXFFST != 0)
	{
		rdataA[rdataA_tail] = SciaRegs.SCIRXBUF.all & 0x00FF;	// 读取数据

		rdataA_tail = rdataA_tail + 1;
		rdataA_tail = rdataA_tail & 0x3F;						// 每存一个字节，循环队列尾指针循环+1
	}
...
interrupt void SCIA_TX_ISR(void)
{
	...
	// Insert ISR Code here
	SciaRegs.SCITXBUF.all = TX_FRAME.SYNC[0];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.SYNC[1];	  // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.FUNC;		  // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[0];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[1];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[2];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[3];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[4];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[5];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[6];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[7];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[8];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[9];     // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[10];	  // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.DATA[11];	  // Send data
	SciaRegs.SCITXBUF.all = TX_FRAME.CSUM;    	  // Send data
