...
float ElecThetaCal(float theta)
{
    ...
    ************************************************************/
    ...
    theta += _2PI * __divf32(deltaposition, CODER_NUM) * POLE_PAIRS;         //角度=两次采样线数差/编码器总线数*2π*极对数
...
float VelocityCal(float velocity)
{
    ...
    ************************************************************/
    ...
    if(EQep1Regs.QEPSTS.bit.UPEVNT == 1)
    {
        ...
        {
            deltatime = EQep1Regs.QCPRD * __divf32(64.0, 200000000);                     //Δt=单位线数内计时*模块时基/200MHz
            if(EQep1Regs.QEPSTS.bit.QDF == 1)
            {
                velocity = __divf32(__divf32(32.0, CODER_NUM), deltatime) * 60;                            //单位线数值/编码器总线数/Δt*60
            }                                                                           //正方向
            else
            {
                velocity = __divf32(__divf32(32.0, CODER_NUM), deltatime) * 60 * -1;                            //单位线数值/编码器总线数/Δt*60
            }                                                                           //负方向
